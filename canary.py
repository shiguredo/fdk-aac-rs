...
def run_cargo_update(dry_run: bool) -> None:
    if dry_run:
        print("Dry-run: Would run 'cargo update shiguredo_fdk_aac'")
    else:
        subprocess.run(["cargo", "update", "shiguredo_fdk_aac"], check=True)
        print("cargo update shiguredo_fdk_aac executed")
